...
export function ProductTerms({ siteConfig, purchaseModel = 'consultation', customSections, className = '' }: ProductTermsProps) {
  ...
  // Sections for consultation model (scooters)
  const consultationSections: TermSection[] = [
    {
      id: 'about',
      title: 'Про замовлення',
      content: `
**Спеціалізований магазин hyper електросамокатів**

Допомагаємо обрати оптимальну модель та оформити замовлення.

**Що ми робимо:**
- Підбираємо модель під ваші потреби та бюджет
- Надаємо об'єктивну інформацію про характеристики
- Допомагаємо оформити замовлення
- Безкоштовна консультація

Ціни на електросамокати орієнтовні. Остаточна вартість та наявність уточнюються після консультації з менеджером.
      `.trim(),
    },
    {
      id: 'consultation',
...
3. **Підтвердження** - Менеджер уточнює актуальну ціну та наявність
4. **Оформлення** - Узгоджуємо деталі замовлення та доставки
5. **Оплата та доставка** - Обираєте зручний спосіб оплати

**Ми пропонуємо:**
- Гарантію виробника (6-24 місяців)
...
- **Оплата карткою онлайн** (Visa/Mastercard через захищену платіжну систему)
- **Передоплата на рахунок ФОП** (банківський переказ)
- **Готівкою при отриманні** у відділенні Нової Пошти (+ комісія перевізника)
- **Безготівковий розрахунок** для юридичних осіб

Після оформлення замовлення менеджер зв'яжеться з вами для підтвердження та узгодження деталей оплати.

**Оплата карткою онлайн:**
- Платіж обробляється миттєво захищеною платіжною системою (стандарт PCI DSS)
- Додатковий захист 3D Secure — підтвердження SMS-кодом від вашого банку
- Ми не збираємо та не зберігаємо дані ваших карток
      `.trim(),
    },
    {
      id: 'prices',
...
Ціни на сайті є орієнтовними та зібрані з публічних джерел.
...
**Точну ціну та умови** уточнюйте у менеджера після консультації.
      `.trim(),
    },
    {
      id: 'warranty',
...
Гарантійне обслуговування надається згідно з умовами виробника.

**Типові умови гарантії:**
- Термін: 6-24 місяців (залежить від бренду та моделі)
...
З усіх питань гарантії зв'яжіться з нами — допоможемо вирішити.
      `.trim(),
    },
  ];
...
Після оформлення замовлення менеджер зв'яжеться з вами для підтвердження та узгодження деталей.
